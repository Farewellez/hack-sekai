...
class reversed_ChaCha20:
    ...
    def encrypt(self, state):
        self._state = bytes_to_words(state)
...
c = reversed_ChaCha20()
# print(c.encrypt(msg_enc), len(c.encrypt(msg_enc))) 20
msg64 = msg[:64]
msg_enc64 = bytes.fromhex(msg_enc)[:64]
final_state = xor(msg64, msg_enc64)
print(final_state)

ks = c.encrypt(final_state)
print(ks)

key = words_to_bytes(ks[4:12])
print(key)
...
flag = p.decrypt(bytes.fromhex(flag_enc), key, bytes.fromhex(iv2))
print(flag)